...
public class TransferFactory {
    ...
    public static void generator(String source, AnActionEvent event, NavigatablePsiElement psiElement, String apiName) {
        actionPerformed(event, source, psiElement, apiName);
    }

    private static void actionPerformed(AnActionEvent event, String source, NavigatablePsiElement psiElement, String apiName) {
    ...
    public static boolean before() {
        ...
        if (StringUtils.equalsIgnoreCase("V3", appSettingService.getState().getVersion())) {
            return MsClientV3.test(appSettingService.getState().getAppSettingStateV3());
        } else {
            return MsClientV2.test(appSettingService.getState().getAppSettingStateV2());
        }
    }
