...
public class UploadSettingPaneV2 extends JBPanel<UploadSettingPaneV2> {
    ...
    // 保存配置
    private final UploadSettingStateV2 appSettingService = Objects.requireNonNull(UploadSettingComponent.getInstance().getState()).getUploadSettingStateV2();
    ...
    public UploadSettingPaneV2() {
        ...
        JLabel tooltip = new JLabel(Bundle.get("ms.upload.message.info"));
        tooltip.setIcon(UIManager.getIcon("OptionPane.informationIcon"));

        // Initialize components
        JLabel serverLabel = new JLabel(Bundle.get("ms.window.tab.server"));
        meterSphereAddress = new JTextField();

        JLabel accessKeyLabel = new JLabel(Bundle.get("ms.window.tab.accessKey"));
        accessKey = new JTextField();

        JLabel secretKeyLabel = new JLabel(Bundle.get("ms.window.tab.secretKey"));
        secretKey = new JPasswordField();

        JLabel workspaceLabel = new JLabel(Bundle.get("ms.window.tab.workspace"));
        workspaceJComboBox = new JComboBox<>();

        JLabel projectLabel = new JLabel(Bundle.get("ms.window.tab.project"));
        projectJComboBox = new JComboBox<>();

        JLabel moduleLabel = new JLabel(Bundle.get("ms.window.tab.module"));
        moduleJComboBox = new JComboBox<>();

        JLabel modeLabel = new JLabel(Bundle.get("ms.window.tab.mode"));
        coverModuleJComboBox = new JComboBox<>();
        jTextField = new JTextField();

        JLabel versionLabel = new JLabel(Bundle.get("ms.window.tab.version"));
        versionJComboBox = new JComboBox<>();

        JLabel updateVersionLabel = new JLabel(Bundle.get("ms.window.tab.updateVersion"));
        updateVersionJComboBox = new JComboBox<>();

        JButton testCon = new JButton(Bundle.get("ms.window.tab.syncUpdate"));

        JPanel auto = new JPanel();
        // Set preferred sizes to match XML constraints
        Dimension labelSize = new Dimension(80, 32);
        ...
        meterSphereAddress.setPreferredSize(new Dimension(300, 32));
        accessKey.setPreferredSize(new Dimension(300, 32));
        secretKey.setPreferredSize(new Dimension(300, 32));
        testCon.setPreferredSize(new Dimension(300, 32)); // Adjust width and height as needed

        // Ensure JComboBox has enough width to display its content
        workspaceJComboBox.setPreferredSize(new Dimension(300, 32));
        projectJComboBox.setPreferredSize(new Dimension(300, 32));
        moduleJComboBox.setPreferredSize(new Dimension(300, 32));
        versionJComboBox.setPreferredSize(new Dimension(300, 32));
        coverModuleJComboBox.setPreferredSize(new Dimension(300, 32));
        updateVersionJComboBox.setPreferredSize(new Dimension(300, 32));
        ...
        // Adding the testCon button next to the secretKey field
        gbc.gridx = 1;
        gbc.gridy = 4; // Next row to keep it after secretKey field
        gbc.gridwidth = GridBagConstraints.REMAINDER;
        gbc.weightx = 1.0;
        settingsTab.add(testCon, gbc);

        gbc.gridx = 0;
        gbc.gridy = 5; // Start new row for other components
        gbc.gridwidth = 1;
        gbc.weightx = 0.0;
        ...
        testCon.addActionListener(actionEvent -> {
            if (MsClientV2.test(appSettingService)) {
                if (initWorkspace()) {
                    DialogTool.info(Bundle.get("ms.upload.message.connection.success"));
                } else {
                    DialogTool.error(Bundle.get("ms.upload.message.workspace.error"));
                }
            } else {
                DialogTool.error(Bundle.get("ms.upload.message.connection.error"));
            }
        });
    ...
    private void initData(UploadSettingStateV2 appSettingService) {
    ...
    private void initProject(UploadSettingStateV2 appSettingService, String organizationId) {
    ...
    private void initCoverModule() {
        ...
        List<CoverModule> list = Arrays.asList(
                new CoverModule("fullCoverage", Bundle.get("ms.upload.message.overwrite")),
                new CoverModule("incrementalMerge", Bundle.get("ms.upload.message.non-overwriting"))
        );
